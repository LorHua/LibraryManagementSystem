...
public class FitPassword extends JInternalFrame{
	private JLabel name;
	...
	public FitPassword () {
		...
		oldPass.setDocument(new Document(50));
		oldPass.addKeyListener(new NumberListener());
		...
		newPass1.setDocument(new Document(50));
		newPass1.addKeyListener(new NumberListener());
		...
		newPass2.setDocument(new Document(50));
		newPass2.addKeyListener(new NumberListener());
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {
					if(newPass1.getText().length()<8||oldPass.getText().length()>50){
						JOptionPane.showMessageDialog(null, "用户密码应在8~50位！");
						return;
					}
					if(newPass2.getText().length()<8||oldPass.getText().length()>50){
						JOptionPane.showMessageDialog(null, "用户密码应在8~50位！");
						return;
					}
				if (oldPass.getText().equals(user.getPsd())) {
					if (newPass1.getText().equals(newPass2.getText())) {
						user.setPsd(newPass1.getText());
						Dao.updatePassword(user.getId(),user.getPsd());
					...
					JOptionPane.showMessageDialog(getContentPane(), "旧密码输入错误");
				}
			}
		});
		button.setText("确认");
